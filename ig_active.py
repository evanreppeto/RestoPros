...
IG_STATUS_TITLE  = "IG Active"   # Status column name
YES_LABEL        = "Yes"
NO_LABEL         = "No"
...
def site_has_instagram(url: str) -> bool:
    """
    Fetch the given URL and look for <a href="...instagram.com/...">
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "instagram.com" in href.lower():
            ...
            print(f"    🔗 found instagram link: {abs_href}")
            return True
    ...
    for meta in soup.find_all("meta"):
        for v in meta.attrs.values():
            if isinstance(v, str) and "instagram.com" in v.lower():
                print(f"    🔗 found instagram in meta: {v}")
                return True
...
def main():
    ...
    col_ig = column_by_title(columns, IG_STATUS_TITLE)
    if not col_ig:
        raise SystemExit(f"Status column '{IG_STATUS_TITLE}' not found.")
    if (col_ig.get("type") or "").lower() != "status":
        raise SystemExit(f"Column '{IG_STATUS_TITLE}' must be a Status column.")

    label_map = parse_status_labels(col_ig.get("settings_str"))
    ...
    for i, it in enumerate(items, 1):
        ...
        current_text = (cvmap.get(ig_col_id, {}) or {}).get("text") or ""
        ...
        has_ig = False
        if website_url:
            has_ig = site_has_instagram(website_url)

        desired_label = YES_LABEL if has_ig else NO_LABEL
        desired_idx   = yes_idx if has_ig else no_idx
        ...
        # update monday
        try:
            change_values(MONDAY_BOARD_ID, item_id, {ig_col_id: {"index": desired_idx}})
