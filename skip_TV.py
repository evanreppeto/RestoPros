...
def classify_with_evidence(pages: List[Tuple[str, str]]) -> Tuple[Set[str], Dict[str, List[str]]]:
    ...
    def top(ev: List[str]) -> List[str]:
        return ev[:12] if DEBUG_EVIDENCE else ev[:5]
...
def main():
    ...
    for idx, it in enumerate(items, 1):
        ...
        # --- SKIP if Target Verticals already has any text (labels present) ---
        current_target_val = col_map.get(target_col_id, "").strip()
        if current_target_val:
            print(f"[{idx}] ⏭️  {display_name}: Target Verticals already set → '{current_target_val}' (skip)")
            continue
